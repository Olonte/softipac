<div class="custom-pagination">
	<ul class="pagination pull-right">
		<c:url var="primeraUrl" value="/paginaCitaInformacion/${paginaInicial}"></c:url>
		<c:url var="ultimaUrl" value="/paginaCitaInformacion/${citas.size()}"></c:url>
		<c:url var="anteriorUrl" value="/paginaCitaInformacion/${indiceActual - 1}"></c:url>
		<c:url var="siguienteUrl" value="/paginaCitaInformacion/${indiceActual + 1}"></c:url>
		<c:choose>
		...
		<c:forEach var="i" begin="${indiceInicio}" end="${indiceFinal}">
			<c:url var="paginaUrl" value="/paginaCitaInformacion/${i}"></c:url>
			<c:choose>
